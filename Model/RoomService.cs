...
namespace BackEnd.Model
{
    public class RoomService
    {
        ...
        public static int AddRoomService(string roomid, string time, string remark, string amount, string status, string employee_id)
        {
            RoomService room_service = Find(roomid, time);
            if (room_service == null)
            {
                return DBHelper.ExecuteNonQuery("INSERT INTO ROOMSERVICE(RoomID,Time,Remark,Amount,Status,EmployeeID) VALUES(:RoomID,:Time,:Remark,:Amount,:Status,:EmployeeID)",
                    new OracleParameter(":RoomID", roomid),
                    new OracleParameter(":Time", time),
